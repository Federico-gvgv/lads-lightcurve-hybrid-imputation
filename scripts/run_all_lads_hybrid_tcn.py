# scripts/run_all_lads_hybrid_tcn.py
...
from src.models.tcn_residual import TCNResidualImputer
...
def masked_mse(y_true: torch.Tensor, y_pred: torch.Tensor, obs_mask: torch.Tensor) -> torch.Tensor:
    # obs_mask: 1 observed, 0 missing. We train only on missing.
...
def eval_loader(model, loader, device: str, L: int, fourier, fourier_mode: str, input_mode: str) -> dict:
    ...
    model.eval()
    for batch in loader:
        y_true_n = batch["y_true"].cpu().numpy()   # (B,L) normalized
        obs_mask = batch["obs_mask"].cpu().numpy() # (B,L)

        mu = batch["mu"].cpu().numpy()  # (B,)
        sd = batch["sd"].cpu().numpy()  # (B,)

        B = y_true_n.shape[0]
        if input_mode == "fourier_aware":
            y_fourier_n = batch["y_fourier_full"].cpu().numpy()
        else:
            # --- Fourier baseline computed here (so it's identical for warm_start=fourier/none)
...
def train_one_star(
    ...
    if warm_start == "none":
        from src.segment_split import TrainEpochDataset, FixedSpecsDataset
        ...
        val_dataset = FixedSpecsDataset(
            star=star, specs=star_build["val_specs"], L=L, min_context=min_context,
            fourier=None, fourier_mode="static", has_gate=False,
        )
        test_dataset = FixedSpecsDataset(
            star=star, specs=star_build["test_specs"], L=L, min_context=min_context,
            fourier=None, fourier_mode="static", has_gate=False,
        )
...
def main():
    ...
    ap.add_argument("--seed_per_star", action="store_true", help="if set, seed = base + 1000*star_index")
    ...
    if args.out_csv is None:
        if args.input_mode == "fourier_aware":
            out_csv = Path(f"outputs/segment_level_fourier_aware/tcn_fourier_aware_{args.fourier_mode}_clean.csv")
        else:
            out_csv = Path(f"outputs/tcn_{args.warm_start}_{args.fourier_mode}.csv")
    else:
