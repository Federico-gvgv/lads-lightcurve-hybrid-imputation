...
torch = None
DataLoader = None
UNet1DResidualImputer = None


def load_runtime_dependencies() -> None:
    global torch
    global DataLoader
    global UNet1DResidualImputer
    ...
    from src.models.unet1d_residual import UNet1DResidualImputer as UNet1DResidualImputer_class

    torch = torch_module
    DataLoader = DataLoader_class
    UNet1DResidualImputer = UNet1DResidualImputer_class


def train_unet_branch(
    train_dataset,
    val_loader,
    device: str,
    seed: int,
...
def train_one_star_calibrated_fusion(
    ...
    print("  Training U-Net none branch...")
    none_model = train_unet_branch(
    ...
    print("  Training U-Net Fourier-aware branch...")
    aware_model = train_unet_branch(
    ...
    row["architecture"] = "UNet1D"
    row["base_channels"] = int(base_channels)
    row["unet_depth"] = int(unet_depth)
    row["kernel_size"] = int(kernel_size)
    row["dropout"] = float(dropout)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="U-Net 1D calibrated fusion between none and Fourier-aware branches."
    )
    ap.add_argument("--data_dir", type=str, default="data/LADS")
    ap.add_argument(
        "--out_csv",
        type=str,
        default="outputs/segment_level_calibrated_fusion_unet1d/unet1d_calibrated_fusion_dynamic_clean.csv",
    )
...
def main() -> None:
    ...
    for si, star_path in enumerate(files):
        ...
        print(
            ...
            f"device={device} | calibrated_fusion=unet1d_none+unet1d_fourier_aware"
        )
        try:
