# scripts/run_all_lads_hybrid_conv_transformer.py
...
from src.models.conv_transformer_residual import ConvTransformerResidualImputer
...
def train_one_star(
    ...
    model = ConvTransformerResidualImputer(
        seq_len=L,
        d_model=d_model,
        nhead=nhead,
...
def main():
    ...
    if args.out_csv is None:
        if args.input_mode == "fourier_aware":
            out_csv = Path(f"outputs/segment_level_fourier_aware/conv_transformer_fourier_aware_{args.fourier_mode}_clean.csv")
        else:
            out_csv = Path(f"outputs/conv_transformer_{args.warm_start}_{args.fourier_mode}.csv")
    else:
